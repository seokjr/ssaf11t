...
export const useJjimStore = defineStore("jjim", () => {
  ...
  const getfriendsJjimListAll = async function (friendid) {
    ...
    await axios.get(`${REST_JJIM_API}/${friendid}`).then((res) => {
  ...
  const getJjimListAll = async function () {
    if (!user.value) return;
    await axios.get(`${REST_JJIM_API}/${user.value.id}`).then((res) => {
  ...
  const toggleJjim = async function (video) {
    const existingJjim = jjimList.value.find(
      (jjim) => jjim.videoId === video.id || jjim.videoId === video.id.videoId
    );
    if (existingJjim) {
