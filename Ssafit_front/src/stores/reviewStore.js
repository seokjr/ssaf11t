...
export const useReviewStore = defineStore("review", () => {
  ...
  const getReviewListAll = async function () {
    await axios.get(`${REST_REVIEW_API}`).then((res) => {
      reviewList.value = res.data;
    });
  };
  const increaseViewCnt = function (review) {
    axios.put(`${REST_REVIEW_API}/${review.reviewId}`).then((res) => {});
  };

  const getReviewList = async function (videoId) {
    await axios.get(`${REST_REVIEW_API}/${videoId}`).then((res) => {
      reviewList.value = res.data;
    });
  };
  ...
  const getUserReviewList = async function (userId) {
    await axios
      .get(`http://localhost:8080/review-api/comment/${userId}`)
      .then((res) => {
        reviewList.value = res.data;
      });
  };
