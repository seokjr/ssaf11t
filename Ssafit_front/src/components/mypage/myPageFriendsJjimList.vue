<template>
    <div>
    <h2 class="text-center">{{route.params.id}}가 찜한 영상 목록</h2>
    <br>
    <div class="container">
      <h4 v-if="currentPageJjimList.length != 0"></h4>
      <h4 v-else class="text-center">찜 목록이 없습니다. 친구보고 영상 좀 보라고 해주세요.</h4>
      <table class="table table-hover text-center" v-if="currentPageJjimList.length != 0">
        ...
        <friendsjjimListItem v-for="Jjim in currentPageJjimList"
...
const currentPageJjimList = computed(()=>{
  return jjimStore.friendjjimList.slice((currentPage.value-1) * perPage, currentPage.value * perPage)
})
